...
Ignition 是一个 JSON 文件，它会且只会被使用一次，`coreos-installer` 会根据它来完成初始化安装环境、设置用户和用户组等任务。相当于我们安装别的 Linux 时在引导程序中配置的内容提前写好，然后安装程序直接对着我们给的配置执行安装。
...
docker run --rm -v ${PWD}:/pwd -w /pwd quay.io/coreos/butane:release \
       --pretty --strict nas.yml > nas.ign
```
